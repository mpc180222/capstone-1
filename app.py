...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql:///instant-gram'
...
# @app.before_request
# def add_user_to_g():
#     """If the user is logged in, they will be added to the Flask global variable"""
...
def show_signup():
    ...
    if form.validate_on_submit():
        ...
        # res = requests.post('http://127.0.0.1:5000/api/users', json=json)
        # new_user_id = res.json()['user']['id']
        res = add_user(json)
        new_user_id = res.json['user']['id']
        new_user = User.query.get(new_user_id)
        do_login(new_user)
        flash(f"Welcome to Instant-Gram, {new_user.username}", 'alert alert-primary')
...
def delete_user(username):
    """Delete the user specified by the username variable. 07/11 """
    user = User.query.filter(User.username == username).one()
...
def follow_unfollow(username):
    """Will create a follow from the Flask global user to the username specified"""
...
def show_home_feed():
    ...
    if CURR_USER_KEY in session and "id" in session:
        ...
        posts = response.json
        # response = requests.get(f'http://127.0.0.1:5000/api/users/{g.user.username}/feed')
        # posts = response.json()
...
def view_comment_likers(commentid):
    ...
    # response = requests.get(f'http://127.0.0.1:5000/api/commentlikes/{commentid}')
    # data = response.json()
